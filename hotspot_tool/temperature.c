						...
						for(j=0; j < model->grid->layers[k].flp->n_units; j++) {
							...
							if (k==3){ 	// Layer0 : In 3Dmem is an SRAM layer its leakage model is different.
									if (leakage[j] == 0)
										...
										power[base+j] = power[base+j] + calc_lc_leakage(model->config->leakage_mode,blk_height,blk_width,temp_first_time[base+j],&model->grid->layers[k].flp->units[j], model->bank_modes);	
										//printf("%f ", power[base+j]);
							}
							else{		// Layer above the base layer in 3Dmem, have a DRAM leakage model.
									if (leakage[j] == 0)
										...
										power[base+j] = power[base+j] + calc_leakage(model->config->leakage_mode,blk_height,blk_width,temp_first_time[base+j],&model->grid->layers[k].flp->units[j], model->bank_modes);
				 					// printf("YES");
							}
							//temp_old[base+j] = temp[base+j]; //copy temp before update
						}
