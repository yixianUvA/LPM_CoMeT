...
def plot_named_combined1(run, name, title, ylabel, smooth=None, force_recreate=False):
    filename = os.path.join(resultlib.find_run(run), '{}.png'.format(name))

    active_cores = resultlib.get_active_cores(run)
    name = 'combine'
    memory_trace = resultlib.get_memory_power_traces(run)
    core_trace = resultlib.get_core_power_traces(run)
    combined_trace = memory_trace + core_trace
    for i in range(0, len(combined_trace)):
        plt.plot(combined_trace[i])
...
def create_plots(run, force_recreate=False):
    ...
    plot_named_traces(run, 'all_power', 'All power', 'Power (w)', lambda: resultlib.get_all_power_traces(run), yMin=0, force_recreate=True)
    ...
    plot_core_trace(run, 'memory_power', 'Memory power', 'Power (W)', lambda: resultlib.get_memory_power_traces(run), active_cores, yMin=0, force_recreate=force_recreate)
    #plot_named_traces(run, 'combine_power', 'Combine power', 'Power (W)', lambda: resultlib.get_combined_power_trace(run), active_cores, yMin=0, yMax=100, force_recreate=force_recreate)
    ...
    plot_named_combined1(run,'hybrid','hybrid data', 'hybrid data')
